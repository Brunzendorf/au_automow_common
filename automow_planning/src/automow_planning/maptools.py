...
def image2array(im):
    ...
    if im.mode == "L":
        a = numpy.fromstring(im.tostring(), dtype=np.uint8)
    else:
...
def rotate_polygon_to(polygon_in, rotation_transform):
    """Takes a polygon and a rotation, returns a rotated polygon"""
    points = np.array(polygon_in.exterior)
...
def zoom_extents(ax, polygons, buff=1.0):
    ...
    min_y = None
    max_y = None
    for polygon_in in polygons:
        bounds = polygon_in.bounds
...
def plot_lines(lines, ax_in=None, color='#6699cc', alpha=1.0):
    import matplotlib.pyplot as pyplot
    show = False
    if ax_in is None:
        ax_in, show = make_axis()
    x, y = lines.line.xy
...
def plot_line(line, ax_in=None, color='#6699cc', alpha=1.0):
    import matplotlib.pyplot as pyplot
    show = False
    if ax_in is None:
        ax_in, show = make_axis()
...
def plot_coords(coords_in, ax_in=None, color='#999999', alpha=1.0):
    show = False
    if ax_in is None:
        ax_in, show = make_axis()
    x, y = coords_in.xy
    ax_in.plot(x, y, 'o', color=color, alpha=alpha, zorder=1)
    if show:
        import pylab
        pylab.show()


def plot_polygon(polygon_in, ax_in=None, color='#999999', alpha=1.0):
    show = False
    if ax_in is None:
        ax_in, show = make_axis()
    x, y = polygon_in.exterior.xy
    ax_in.plot(x, y, color=color, alpha=alpha)
    ax_in.set_xlim(min(x) - 1, max(x) + 1)
    ax_in.set_ylim(min(y) - 1, max(y) + 1)
    ax_in.set_aspect(1)
    if show:
        import pylab;
        pylab.show()
