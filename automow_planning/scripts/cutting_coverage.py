...
class CuttingCoverage(object):
    ...
    def __init__(self):
        ...
        # ROS params
        self.cutter_frame_id = rospy.get_param("~cutter_frame_id", "cutter")
    ...
    def update_thread(self):
        ...
        while not rospy.is_shutdown():
            ...
            try:
                ...
                new_cells = self.update_coverage_map(cutter)
                self.grid_cells_lock.acquire()
    ...
    def update_coverage_map(self, cutter_in):
        """
        Takes the cutter shapes and then updates the coverage map.
        """
        new_cells = []
        # Add the cutter
        if self.cutter_state:  # If the left cutter is on
            points = self.get_raster_shape(cutter_in,
    ...
    def setup_raster_shape(self, cutter, resolution=100.0):
        """
    ...
    def get_raster_shape(self, cutter, resolution=100.0):
        """
        Returns the locations, as Point's, of pixels that represent the cutter.
        """
        offset = (cutter.bounds[0], cutter.bounds[1])
        if self.cutter_pixels is None:
